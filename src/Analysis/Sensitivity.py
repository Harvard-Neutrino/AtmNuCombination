...
def sensitivity(analysis, t12, t13, t23, dm21, dm31, dcp, Ordering, experiments, outfile):
	...
	if analysis.NoSyst:
		...
		if analysis.SystNominalList == analysis.SystPrior:
			for source in analysis.Systematics:
				if len(analysis.Systematics[source])>0:
					...
					bounds = tuple(bnds)
					res = minimize(BlockChi2SystsCombined, analysis.SystNominal[source], args=(analysis, Obs, experiments, source), method='L-BFGS-B', jac=True, bounds=bounds, options={'disp' : False, 'ftol' : 1e-2, 'gtol': 1e-01})
