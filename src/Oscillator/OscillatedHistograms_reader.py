...
from AtmGridOscillator import AtmOsc
...
with h5py.File(infile, 'r') as hf:
	...
	print('-----------------------------')
	print('Extracting binned plots for:')
	print('-----------------------------')
	...
	# Build path to histograms
	path = '/sin^2(Theta23)='+str(round(th23,2))+'/DeltaM32(eV^2)='+str(round(dm32,4))+'/sin^2(Theta13)='+str(round(th13,3))+'/DeltaCP='+str(round(dcp,2))+'/'+nmo
	# print(path)
	par_location = hf[path]
	for i in samples:
		H = np.array(par_location[samples[i]+' Events'][()])
		cz = np.array(par_location[samples[i]+' Zenith bins'][()])
		ev = np.array(par_location[samples[i]+' Energy bins'][()])
		H = H.T
		X, Y = np.meshgrid(cz, ev)
		fig = plt.pcolormesh(X, Y, H)
		plt.colorbar(fig)
		plt.title(samples[i])
		plt.xlabel(r"$\cos(\theta_{zen})$")
		plt.ylabel(r"$E_{\nu}$ (GeV)")
		plt.savefig('figs/'+samples[i]+'.png')
		plt.clf()
