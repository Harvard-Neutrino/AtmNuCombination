...
def IC():
	hi = 125
	lo = 1.
	binnum = 51 # for 1-50 I use 51, for 1-125 I use 61 or 71
	...
	bin_width_i = np.digitize(ICMC["true_energy"][nue_mask & cc_mask], energy_bins_fine) - 1 - 1
	widths = (energy_bins_fine[1:] + energy_bins_fine[:-1])/2

	bin_weights = np.asarray([widths[i] for i in bin_width_i])

	print("start binning")
	# use numpy to bin the data
	ehist, ebin = np.histogram(ICMC["true_energy"][nue_mask & cc_mask], bins=energy_bins_fine, \
		weights=V_eff[nue_mask & cc_mask]/bin_weights)
...
def IC_vol_check():
	...
	print("non-density nu_e histogram integral area is, ", e_density_integral)
	print("non-density nu_ebar histogram integral area is, ", ebar_density_integral)
	print("non-density rescaled nu_e histogram integral area is, ", rescaled_e_density)
	print("non-density rescaled nu_ebar histogram integral area is, ", rescaled_ebar_density)
