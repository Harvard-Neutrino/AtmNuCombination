...
res_e_true, res_e_reco, res_zen_true, res_zen_reco, res_W_mc, topology, pdg = fakegen.generate()

if write:
	df_array = np.ndarray((7, len(res_e_true)))
	df_array[0] = topology
	df_array[1] = pdg
	...
	df_array[6] = res_W_mc
	df = pd.DataFrame(df_array.T, columns = ["pid", "pdg", "true_energy", "reco_energy", "true_zenith", "reco_zenith", "weight"])
	df.to_csv("ORCA.csv")