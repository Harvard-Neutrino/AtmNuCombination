...
res_e_true, res_e_reco, res_zen_true, res_zen_reco, res_W_mc, topology = fakegen.generate()

if write:
	df_array = np.ndarray((6, len(res_e_true)))
	df_array[0] = topology
	df_array[1] = res_e_true
	df_array[2] = res_e_reco
	df_array[3] = res_zen_true
	df_array[4] = res_zen_reco
	df_array[5] = res_W_mc
	df = pd.DataFrame(df_array.T, columns = ["pid", "true_energy", "reco_energy", "true_zenith", "reco_zenith", "weight"])
	df.to_csv("ORCA.csv")